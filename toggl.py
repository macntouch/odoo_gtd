...
# Definitions
ODOO_HOSTNAME = 'localhost'  # Odoo hostname
ODOO_DATABASE = 'odoo9_gtd'  # Odoo database name
ODOO_USER = 'admin'  # Odoo user name
ODOO_TIMEZONE = 'Europe/Chisinau'  # Timezone
TOGGL_API_TOKEN = '801d83bb8f3a63c492183911f31e1bc2'  # please complete with Toggl API token
TOGGL_API_URL = 'https://www.toggl.com/api/v8/'
TOGGL_REPORTS_URL = 'https://toggl.com/reports/api/v2/'
TOGGL_WORKSPACE = 'Nibbana'  # Toggl workspace to process
TOGGL_USER_AGENT = 'Litnimax <litnimaxster@gmail.com>'  # This is required by Toggl
...
if args.username:
    ...
    odoo_username = 'admin' #raw_input('Odoo User: ')

if args.password:
    odoo_password = args.password
else:
    odoo_password = 'admin' #getpass.getpass('Odoo Password: ')
...
project_model = connection.get_model('gtd.project')
active_projects = project_model.search(['state', '=', 'Active'])
print active_projects

"""

# Add Oddo tasks as Toggl projects and create dictionary with ids to use later.
task_model = connection.get_model('gtd.task')
